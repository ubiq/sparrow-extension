...
function injectScript(content) {
  try {
    ...
    console.error('Sparrow: Provider injection failed.', error);
  }
}
...
async function setupStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('Sparrow Inpage Multiplex', err),
  );
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('Sparrow Background Multiplex', err);
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('Sparrow Legacy Inpage Multiplex', err),
  );
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('Sparrow Background Legacy Multiplex', err);
      notifyInpageOfStreamFailure();
    },
  );
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Sparrow: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  );
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Sparrow: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `Sparrow: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
...
function redirectToPhishingWarning() {
  console.debug('Sparrow: Routing to Phishing Warning component.');
